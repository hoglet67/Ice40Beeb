...
#include "beeb_roms.h"
#define  BEEB_ROMS_START 0x00C000

#define VER "0.35 Ice40Beeb"
...
void send_beeb_roms() {
   uint32_t start = BEEB_ROMS_START;
   uint32_t end   = BEEB_ROMS_START + beeb_roms_bin_len - 1;

	// Additional code to send the Beeb ROMs (16KB total)
	gpio_high(ICE40_SPI_CS);
	HAL_Delay(100);
	uart_puts("Sending Beeb ROMS\n");
	gpio_low(ICE40_SPI_CS);
	spi_write((uint8_t *)&start, 3);
	spi_write((uint8_t *)&end, 3);
	spi_write((uint8_t *)&beeb_roms_bin[0], beeb_roms_bin_len);
	gpio_high(ICE40_SPI_CS);
	uart_puts("Done\n");
}
